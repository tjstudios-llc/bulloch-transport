...
def render_driver_maps_page():
    ...
    with ui.header().classes('bg-blue-900 text-white px-3 py-1.5 justify-between items-center shadow-md h-11 shrink-0'):
        ...
        with ui.row().classes('items-center gap-2'):
            # Render Navigation / Admin buttons
            ui.button('🏠 Dashboard', on_click=lambda: ui.navigate.to('/driver')).props('dense flat color=white text-color=white').classes('text-xs')
    ...
    with ui.column().classes('w-full max-w-[1024px] h-[556px] p-2 bg-slate-100 justify-between overflow-hidden mx-auto gap-2'):
        ...
        with ui.row().classes('w-full bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-200 items-center justify-between shrink-0'):
            ...
            if active_route:
                ui.label(f"{active_route.get('name')} ({active_route.get('shift')} Shift)").classes('text-xs font-bold text-slate-800')

        # --- Fallback State: No Active Route Assigned ---
        if not active_route:
            with ui.card().classes('w-full flex-1 p-6 text-center bg-amber-50 border border-amber-200 rounded-xl justify-center items-center'):
                ui.label(f"⚠️ No active route assigned to Bus #{active_bus}.").classes('text-base font-bold text-amber-800')
                ui.label("Please verify the bus number above or contact dispatch.").classes('text-xs text-amber-600')
            return
        ...
        # --- Main Split View Layout (Manifest + Leaflet Map) ---
        with ui.row().classes('w-full flex-1 gap-2 items-stretch overflow-hidden min-h-0'):
            
            # --- LEFT CARD: Route Manifest Column (Scrollable List) ---
            with ui.card().classes('w-80 bg-white p-2.5 rounded-xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden shrink-0 gap-2'):
                ...
                # Scrollable Stop Items Container
